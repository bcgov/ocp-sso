...
public class TwoWayClaimToGroupMapper extends AbstractClaimMapper {

    public static final String[] COMPATIBLE_PROVIDERS = {KeycloakOIDCIdentityProviderFactory.PROVIDER_ID, OIDCIdentityProviderFactory.PROVIDER_ID};

    private static final List<ProviderConfigProperty> configProperties = new ArrayList<ProviderConfigProperty>();

    private static final String GROUP_NAME = "group-prefix";
	  private static final String DEFAULT_GROUP = "github:bcgov";
    ...
    static {
        ...
        property.setLabel("Group name");
        property.setHelpText("Group the user will join if claim matches.");
        property.setDefaultValue(DEFAULT_GROUP);
        property.setType(ProviderConfigProperty.STRING_TYPE);
        configProperties.add(property);

// TODO: How to get list of 
        // property.setName(ConfigConstants.GROUP);
        // property.setLabel("Group");
        // property.setHelpText("Group for user to join if claim is present.  Click 'Select Group' button to browse groups, or just type it in the textbox.");
    ...
    public void importNewUser(KeycloakSession session, RealmModel realm, UserModel user, IdentityProviderMapperModel mapperModel, BrokeredIdentityContext context) {
        ...
        // Join group if claim match, otherwise leave:
        if (hasClaimValue(mapperModel, context)) {
            user.joinGroup(group);
            effectiveMembership.put(groupName, true);
        } else {
            if (effectiveMembership.getOrDefault(group.getName(), false) != true) {
